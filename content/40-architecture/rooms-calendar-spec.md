...
**Решение, сужающее FR-17 AC2 (заблокирована по требованию).** У `get_room` нет известного
каталога ошибок — в отличие от календаря (Ф-26), единственный подтверждённый исход живым зондом
это 200; поведение для несуществующей комнаты не проверено. Поэтому **любой** отказ (включая
гипотетический 404) сегодня уходит в корреляционную диагностику, а не классифицируется тихо как
«комната не найдена» — иначе реальный дрейф контура рисковал бы маскироваться тем же кодом,
которым сервер, возможно, легитимно отвечает на отсутствующее имя. Сужение (собственный каталог
для `get_room`, как у календаря) — предмет следующей ревизии после разблокировки FR-17 AC2 боевой
проверкой, не выполняется в этой волне.
