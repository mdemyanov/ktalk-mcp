...
### 5.3a Включительная граница `end` (FR-39, ADR-017)

Требование FR-39 меняет семантику `--end`/`end` инструмента на включительную; сервер остаётся
полуоткрытым `[start 00:00, end 00:00)` (Ф-60 RES-004) — трансляция происходит на границе клиента.

**Где.** В `_fetch_segment`, не в `split_window`. `split_window` продолжает работать в
включительных календарных датах — тех же, что видит вызывающий, и тех же, что репортятся в
`incomplete_segments`; её арифметика 7-дневного лимита (см. ниже) не должна знать о серверной
полуоткрытости. Пересчёт «включительная дата → то, что уходит в параметр `end`» — обязанность
самого тонкого слоя у запроса, симметрично тому, как этот же слой уже решает `take=100` и
`roomName`.

```python
params["end"] = (seg_end + timedelta(days=1)).isoformat()
```

**Как именно.** Голая дата `seg_end + 1 день`, не `seg_end` с явным временем конца суток. Ф-62
показывает, что сервер время принимает, но не какой часовой базис (UTC/MSK) служит отсечкой;
явное время в `end` полагалось бы на нерешённый вопрос. Голая дата `+1` не решает вопрос базиса
тоже, но и не зависит от ответа: Ф-60 замерил ровно этот механизм (бинарная дата в `end`, инкремент
на календарный день добавляет ровно один календарный день выдачи) на двух независимых стыках без
расхождений, при любом из двух гипотетических базисов полуночи — граница ложится в один и тот же
пустой промежуток (Ф-62). FR-39 работает на уровне календарных дат, не долей суток, поэтому этой
устойчивости достаточно; отдельная клиентская отсечка лишнего хвоста по дате не нужна —
преждевременная сложность без замера, который показал бы, что она чем-то оправдана.

**7-дневный лимит (Ф-63).** `split_window(max_days=7)` не меняется: сегмент `[seg_start,
seg_start+6]` (7 календарных дат, инвариант как раньше). На проводе после `+1` уходит
`end = seg_start+7`, разница `(end − start).days == 7` — ровно на пороге `≤ 7`, тот же диапазон,
что уже проверен Ф-63/Ф-20 (`08-01→08-08`, разница 7, 200). `max_days` не понижается до 6:
понижение отдало бы на проводе разницу 6 вместо доступных 7, недобрав день лимита без причины.

**Потеря дня на стыке.** Устраняется тем же изменением, без правки `split_window`. День `seg_end`
раньше не попадал ни в текущий сегмент (сервер отсекал `end` исключающе), ни в следующий (тот
начинается с `seg_end + 1`). Теперь `end` на проводе — `seg_end + 1`, значит `[seg_start,
seg_end+1)` включает весь день `seg_end`; следующий сегмент по-прежнему начинается с `seg_end + 1`
— без перекрытия и без дыры. Дедуп по `id`/`(meetId, start)` (§5.3, уже есть) остаётся
подстраховкой на случай, если сервер когда-нибудь начнёт отдавать перекрывающиеся сегменты, не
механизмом, на который в норме полагается склейка.

**`start > end` — отказ до сети.** Точка проверки — `split_window`, единственная общая точка входа
обеих поверхностей (`get_calendar_window` вызывается и из `cli_meetings_read._list_calendar`, и из
`tools_meetings.ktalk_list_calendar`). Раньше `split_window` пропускала `start > end` как есть с
комментарием «сервер сам ответит 400» — этот путь отменяется: комментарий и ветка передачи
как есть убираются, вместо них `raise KTalkError(...)` с собственным текстом («начало окна позже
конца»), не пересказом серверного «Дата окончания должна быть больше даты начала» (AC5 FR-39).
Тип исключения — `KTalkError`, не голый `ValueError`: в `_run` (CLI) тип не важен — там общий
`except Exception`, а вот `ktalk_list_calendar` (MCP) ловит только `_AUTH_ERRORS = (KTalkError,
KTalkConfigError)` — `ValueError` этим блоком не перехватывается и ушёл бы наружу
нередактированным трейсбеком, в отличие от уже установленного контролируемого пути ошибки. CLI:
код возврата 1 — существующий путь `_run`'а (`except Exception`, `redact_secrets`, `return 1`),
не новый код; отличать от кода усечения аргументов (2, `cli.py`) не нужно — AC5/AC6 требуют
только «код ≠0, отличный от честного 0», не отдельный код под каждый класс ошибки.
...
- FR-39/ADR-017 (`split_window`/`_fetch_segment`, §5.3a): однодневное окно `--start D --end D` в
  начале, середине и на последнем дне многодневного диапазона — три отдельных вызова, три разные
  фикстуры (AC3). Окно шире 7 дней (17–30, разница 13) — встречи каждого дня, включая оба дня на
  стыке соседних сегментов, без потерь и без дублей по `id` (AC2). `start > end` — отказ до сети:
  мок клиента подтверждает отсутствие обращения, текст сообщения свой, не серверный (AC5). Два
  теста на разные коды возврата: честный пустой день (0, `items=[]`) против отклонённого обратного
  интервала (≠0) — не один и тот же код (AC6). CLI и MCP на одном и том же однодневном окне дают
  идентичный результат (AC1/AC7 — оба вызывают `get_calendar_window`).
...
| `split_window` — `start > end` отказ, границы `+1` на `_fetch_segment` (FR-39/ADR-017, AC1–AC6) | unit | чистая арифметика дат + мок-транспорт httpx на `params["end"]` |
...
**Дополнение — FR-39/ADR-017 (включительная граница `--end`).** Реализуется поверх §5, отдельного
шага в порядке выше не добавляет (правка внутри `calendar_reader.py`, тот же файл, что шаг 4):
1. `split_window` — заменить ветку «`start > end` передаётся как есть» на `raise KTalkError(...)`
   с собственным текстом (§5.3a); `max_days` не менять.
2. `_fetch_segment` — `params["end"] = (seg_end + timedelta(days=1)).isoformat()` вместо
   `seg_end.isoformat()`.
3. Ревизия навыка `ktalk-meetings` на компенсацию полуоткрытости на его стороне — обязательный шаг
   внедрения (ADR-017 «Негативные»/«Смягчения»), не опция.

**Сценарии приёмки FR-39:** AC1–AC6 (см. §5.3a выше и `rooms-calendar-scheduling.md`, `### FR-39`).
...
- FR-39/ADR-017: после внедрения смоук-прогон дополняется однодневным окном `--start D --end D` на
  боевом аккаунте — убедиться, что встречи известного дня возвращаются (обратная совместимость не
  сохраняется, ADR-017 «Негативные»; вызывающие, включая навык `ktalk-meetings`, должны быть
  ревизованы до раскатки, не после).
