...
Проверено вызовом функции (`render_transcript_output` + envelope-сборка §«Оркестрация» выше), не
рассуждением: сверка по умолчанию меняет форму `get-transcript --json` для ЛЮБОГО ответа —
цельного, чанкированного и вне диапазона чанка не меняет (см. edge case «Оркестрация» п.4).

**До (1.x, любой вызов с `--json`):**
```json
{"result": "…текст чанка…", "chunk": 1, "total_chunks": 3, "has_more": true}
```
(для нечанкированного ответа — сам `TalkTranscript`: `{"status": …, "tracks": […], …}`.)

**После (2.0.0, умолчание — сверка включена):**
```json
{"transcript": {"result": "…", "chunk": 1, "total_chunks": 3, "has_more": true},
 "identity_check": {"result": "match", "transcript_speakers": […], "recording_participants": […]}}
```

**После, `--no-verify-identity`:** форма 1.x без изменений — `identity_check` отсутствует, тело
не оборачивается.

**Кого ломает:** любого потребителя, документированно разбирающего поля верхнего уровня `--json`
без предварительного разворота `transcript`. Поимённо — `agents/ktalk-processor.md` репозитория
`ktalk-plugin` (строки 133-137 на момент этого решения), читающий `result`/`chunk`/`total_chunks`/
`has_more` корня; первый же вызов на реальной записи получит эти ключи под `transcript`, а не в
корне, и не найдёт их там, где ожидал.

**Путь миграции:**
1. Читать тело из `transcript`, не из корня ответа — постоянное исправление, переживает волну.
2. Временная альтернатива на период до правки потребителя: `--no-verify-identity` возвращает
   форму 1.x без изменений — обменивает сверку на совместимость на вызов.

**Версия пакета:** **2.0.0**, не 1.1.0 — смена формы документированного `--json` по умолчанию
(без явного отказа) есть breaking change по контракту, а не аддитивная возможность. Владелец
рассмотрел и отклонил неломающий вариант (соседний ключ рядом с телом, а не обёртка) — форма
envelope выбрана намеренно, не по недосмотру. Правка `ktalk-processor.md` под `transcript` —
задача той же волны, не последующая.
...
- Версия пакета — **2.0.0** (breaking change выше, не аддитивный релиз).
