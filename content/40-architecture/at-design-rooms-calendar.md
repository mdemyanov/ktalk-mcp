...
## Ревизия FR-40 (эпик ktalk-mcp-bd1, QA-011, 2026-08-19) — формат `--timezone`

Дополняет секцию FR-13 выше (компоновщик `build_meeting_body`). Отдельный failing-stub-файл
`tests/test_fr40_timezone_format.py` (36 тестов: 31 стаб + 5 регрессионных guard'ов уже
зелёных сегодня — `GMT+0`/`GMT+14`/`GMT-14` и `timezone=None` уже ведут себя так, как требует
ADR-020, до самой реализации: диапазон и обязательность-раньше-формы совпадают с текущим
поведением случайно, `TimezoneFormatError` для них не заявлена). Архитектура — [ADR-020
spec](../00-project/adr/ADR-020-timezone-format-validation.md) ([companion-спека с кодом
регулярки/класса ошибки](ADR-020-timezone-format-validation-spec.md)).

| AC ID | Формулировка (кратко) | Assertion outline | Тип | Тест-функция | Статус |
|---|---|---|---|---|---|
| FR-40 AC-1 | `GMT+3` принят, тело уходит без изменений | снимок тела: `timezone == "GMT+3"`, прочие поля не мутированы | unit | `test_ac1_gmt_plus_3_accepted_body_unchanged_snapshot` | green (guard) — регулярки ещё нет, `build_meeting_body` и так пропускает любую строку |
| FR-40 AC-2 | Семь отклонённых форм замера → `TimezoneFormatError` до сети | `pytest.raises(TimezoneFormatError)` (unit) + `httpx_mock.get_requests() == []` на обеих CLI-поверхностях | unit + integration | `test_ac2_rejected_form_raises_timezoneformaterror_unit[...]` (×7), `test_ac2_preview_surface_rejects_before_any_network_call[...]` (×7, `cli_meeting.py`), `test_ac2_confirm_surface_rejects_before_any_network_call[...]` (×7, `cli_meeting_confirm.py`) | red (stub) |
| FR-40 AC-3 | Отказ по формату не расходует бюджет санкции и не выдаёт `confirmation_id` | хранилище подтверждений пусто после отказа; `write_sanction.read_state(...).remaining` не меняется до/после | unit + integration | `test_ac3_format_rejection_issues_no_confirmation_id`, `test_ac3_format_rejection_does_not_consume_write_sanction_budget` | red (stub) |
| FR-40 AC-4 | Сообщение называет формат и пример, не сырой `CalendarTimeZoneParse` | текстовый снимок: `"GMT+3" in message`, `"CalendarTimeZoneParse" not in message` | unit | `test_ac4_error_message_names_format_and_example_not_raw_server_text` | red (stub) |
| FR-40 AC-5 | `--help` CLI и докстринг `build_meeting_body` называют формат явно | `"GMT+3" in parser.format_help()`, `"GMT+3" in inspect.getdoc(build_meeting_body)` | unit | `test_ac5_cli_help_names_timezone_format`, `test_ac5_build_meeting_body_docstring_names_timezone_format` | red (stub) |

**Boundary cases FR-40 (сверх дословных AC, ADR-020 §3/companion-спека «Edge cases»):**
- Экстраполированная граница диапазона `0..14` на обеих сторонах знака: `GMT+0`, `GMT+14`,
  `GMT-14` приняты — `test_boundary_regex_accepts_edge_of_declared_range` (параметризован).
- За пределами диапазона (`GMT+15`, `GMT+50` — второе взято из отклонённого варианта issue с
  диапазоном `0..99`, который эту форму пропустил бы) и структурно некорректные (`GMT3` без
  знака, `gmt+3` в нижнем регистре, пустая строка) отклонены —
  `test_boundary_regex_rejects_out_of_range_or_malformed` (параметризован, 5 значений).
- Порядок проверок: `timezone=None` даёт `MissingFieldError`, не `TimezoneFormatError` —
  `test_boundary_timezone_none_raises_missing_field_not_timezone_format` (уже green:
  `_REQUIRED`-цикл существует и отрабатывает раньше любой будущей проверки формы).
- Регистр (`gmt+3`) и обязательность знака (`GMT3`) регулярка отклоняет однозначно по
  companion-спеке ADR-020 (§ «Edge cases»: «регулярка регистрозависима, `gmt+3` отклоняется») —
  не открытый вопрос вопреки предположению в постановке задачи QA-011, решение SA уже
  зафиксировано текстом кода регулярки, вопрос к SA не заводится.

**Error cases FR-40 — оба класса, явно:**
- **Испорченный/опечатанный ввод** (целевой пользователь передал нотацию, которую сам считает
  стандартной, но сервер/локальная проверка её не распознают): все семь форм замера AC-2 плюс
  `GMT3`/`gmt+3`/пустая строка в boundary-тесте — не идентификатор/URL-сегмент/поисковый
  запрос буквально, но тот же класс «опечатанного» пользовательского ввода поля с фиксированным
  форматом.
- **Замаскированный отказ** (система молча принимает вход или подставляет дефолт вместо
  наблюдаемой ошибки): ADR-020 явно отклонил вариант с молчаливой нормализацией
  (`Europe/Moscow`→`GMT+3` и подобные, FR-40 «Решение», ADR-020 «Alternatives Considered») —
  прямого сценария «система тихо принимает и переформатирует» в контракте нет, поэтому явный
  `N/A` для отдельного теста «нормализация не происходит»: он логически покрыт уже AC-2
  (`TimezoneFormatError` вместо тихого пропуска — если бы нормализация произошла, регулярка
  проверяла бы результат нормализации и AC-2 не упал бы на входных строках, а он обязан упасть
  на них самих). Второе проявление класса — реальный контракт из ADR-020 §5: значение внутри
  диапазона `0..14`, но отличное от `GMT+3` (например `GMT+5`), проходит локальную проверку и
  «молча» уходит на сеть, где сервер может ответить `CalendarTimeZoneParse` тем же путём, что
  до этого решения — ADR-020 сознательно НЕ устраняет этот случай (нет наблюдённого замера),
  поэтому тест на него не пишется здесь: заведение такого теста означало бы придумывать
  ожидание, не выведенное ни требованием, ни решением SA. Отмечено, не молча пропущено.

**Прогон на момент написания (до реализации Dev):** `uv run --with pytest-xdist --with pytest
pytest tests/ -q -n 8` — 31 failed (стабы `test_fr40_timezone_format.py`), 519 passed (весь
остальной набор, включая 5 регрессионных guard-тестов этого же файла и всё, что было раньше,
не тронуто). До этого файла — 514 passed, 0 failed.

**Вопросы к SA — не заведены.** Обе точки, которые постановка задачи заранее пометила как
потенциально открытые (регистр, обязательность знака), оказались уже решены явным текстом
companion-спеки ADR-020 (код регулярки + раздел «Edge cases») на момент чтения — не
самостоятельная догадка этого файла. Единственный оставшийся открытый край — поведение сервера
на значениях внутри `0..14`, отличных от `GMT+3` (например `GMT+5`) — ADR-020 §3/§5 сам называет
его неизмеренным и сознательно не устраняет; тест на него не заводится по той же причине, по
которой сам ADR не решает вопрос (anti-fabrication).
