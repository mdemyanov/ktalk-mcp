...
- ~~**URL git-репозитория ktalk-plugin.**~~ **Закрыт 2026-08-18 владельцем.**
  Репозиторий создан и опубликован:
  `https://doc-hub.gitlab.yandexcloud.net/tools-ai/ktalk-plugin`, видимость
  **public** (по прецеденту `nauta`; соседние плагины группы `tools-ai` —
  private, выбор сделан владельцем явно). `.claude-plugin/marketplace.json`
  не менялся: source остаётся `"./"` — манифест самоописывающий, адрес живёт
  в remote, не в файле. Установка — `/plugin marketplace add <url>` +
  `/plugin install ktalk@ktalk-plugins`, задокументирована в README плагина.
  Перед публикацией дерево и вся git-история повторно проверены на секреты,
  абсолютные пути и раскладку vault'а — совпадения только в паттернах самого
  скрипта проверки и в тексте коммита.
